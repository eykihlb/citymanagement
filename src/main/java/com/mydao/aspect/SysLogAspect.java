package main.java.com.mydao.aspect;
...
public class SysLogAspect {
	 ...
	 */
	@Pointcut("execution(* main.java.com.mydao.controller..*.*(..))")
	 ...
	 * @param keys
	 */
	@AfterReturning(value = "execution(* main.java.com.mydao.controller.SysUserController.hello(..))",returning = "keys")
	public void afterReturningAdvice(JoinPoint joinPoint,String keys){
		...
		try {
			Map<String,Object> paramMap = JsonUtil.jsontoHashMap(keys);
			Map<String,Object> paramMap1 = JsonUtil.jsontoHashMap(paramMap.get("data").toString());
			LoginLog ll = new LoginLog();
			ll.setIdate(new Date());
			ll.setRecId(1L);
			ll.setLoginTime(new Date());
			ll.setStaffId(paramMap1.get("staffId").toString());
			ll.setTerminalNo("111");
			//System.out.println(loginLogService.selectByPrimaryKey(1L).getStaffId());
			System.out.println(loginLogService.updateByPrimaryKeySelective(ll));
		}catch (Exception e){
			e.printStackTrace();
		}
	 ...
	 */
	@Around("execution(* main.java.com.mydao.controller..*.*(..))")
